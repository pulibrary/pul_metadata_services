module PulMetadataServices
  module ExternalMetadataSource
    ...
    module ClassMethods
      ...
      def title_from_ead(record)
        record = negotiate_ead(record)
        crumbs = record.xpath('//pulfa:context//pulfa:breadcrumbs/pulfa:crumb',
      ...
      def publisher_from_ead(record)
        record = negotiate_ead(record)
        [record.xpath('//pulfa:context/pulfa:collectionInfo/pulfa:collection-creators').first.text.strip]
      end

      # !Needs Work
      # could not get the non namespaced element corpname in
      # pulfa:context to return a value
      # using xpath so left in a shortcut here.
      def creator_from_ead(record)
        record = negotiate_ead(record)
        [record.xpath('//pulfa:context/pulfa:collectionInfo/pulfa:collection-creators').first.text.strip]
      end

      # !Needs Work
      # could not get the non namespaced element unitdate in
      # pulfa:content to return a value
      # using an xpath.
      def date_from_ead(record)
        record = negotiate_ead(record)
        #record.xpath('//pulfa:context/pulfa:collectionInfo')
        [record.at_css('unitdate').text]
      end 
      ...
      def negotiate_ead(record)
        if record.instance_of? Nokogiri::XML::Document
          record
        else
          Nokogiri::XML(record)
        end
  ...
  end # closing ExternalMedataSource
end # closing PulStore
